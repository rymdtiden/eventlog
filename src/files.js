...
function findLogfiles(filenameTemplate) {
					...
					return promise
						...
						.then(stat => {
							if (stat.isDirectory()) {
								...
								const date = logfileDate(path.join(dir, file), filenameTemplate);
								//, path.join(dir, file), typeof date);
								if (date !== false) {
									dateFileMap[date.year * 10000 + date.month * 100 + date.day] = path.join(dir, file);
								}
							}
						})
						.catch(() => {});
	...
	return recursiveFileSearch(baseDir)
		.then(() => Object.keys(dateFileMap).sort().map(key => dateFileMap[key]));
}
...
function filenameTemplateInfo(filenameTemplate) {
	...
	const yOrder =
		yPos < mPos && yPos < dPos ? 1 :
		yPos < mPos && yPos > dPos ? 2 :
		yPos > mPos && yPos < dPos ? 2 : 3;
	const mOrder =
		mPos < yPos && mPos < dPos ? 1 :
		mPos < yPos && mPos > dPos ? 2 :
		mPos > yPos && mPos < dPos ? 2 : 3;
	const dOrder =
		dPos < yPos && dPos < mPos ? 1 :
		dPos < yPos && dPos > mPos ? 2 :
		dPos > yPos && dPos < mPos ? 2 : 3;
	...
	const matchSubstrings = [
		...
		},
		{
			...
			length: filenameTemplate.indexOf(secondMatch) - filenameTemplate.indexOf(firstMatch) - 2
		},
		{
			start: filenameTemplate.indexOf(secondMatch) + 2,
			length: filenameTemplate.indexOf(thirdMatch) - filenameTemplate.indexOf(secondMatch) - 2
		},
		{
...
function logfileDate(filename, filenameTemplate) {
	...
	const substrsMatching = templateInfo.matchSubstrings.reduce((valid, substr) =>
		valid && filename.substr(substr.start, substr.length) === substr.str, true);
...
function existingLogfileByPosition(position, filenameTemplate) {
	...
	return findLogfiles(filenameTemplate)
		.then(files => {
...
function validatedDate(year, month, day) {
	...
	if (day < 1 || day > [ 31, ((year % 4 === 0) && (year % 100 !== 0)) || (year % 400 === 0) ? 29 : 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 ][month - 1]) throw new InvalidDayError();
...
function logfileForDate(y, m, d, filenameTemplate) {
	...
	const templateInfo = filenameTemplateInfo(filenameTemplate);
	return templateInfo.matchSubstrings[0].str +
			(templateInfo.firstMatch === "%y" ? year : ("00" + (templateInfo.firstMatch === "%m" ? month : day)).slice(-2)) +
			templateInfo.matchSubstrings[1].str +
			(templateInfo.secondMatch === "%y" ? year : ("00" + (templateInfo.secondMatch === "%m" ? month : day)).slice(-2)) +
			templateInfo.matchSubstrings[2].str +
			(templateInfo.thirdMatch === "%y" ? year : ("00" + (templateInfo.thirdMatch === "%m" ? month : day)).slice(-2)) +
...
function nextExistingLogfile(afterLogfile, filenameTemplate) {
	...
	try {
		...
		log("Invalid afterLogfile when trying to nextExistingLogfile(): %s", afterLogfile);
	...
	return findLogfiles(filenameTemplate)
		.then(logfiles => {
			const dateFileMap = logfiles.reduce((dateFileMap, logfile) => {
				...
				if (filedate.year > year || (filedate.year === year && filedate.month > month) || (filedate.year === year && filedate.month === month && filedate.day > day)) {
					return {
						...dateFileMap,
...
function touch(filename) {
	...
	return mkdir(path.dirname(filename), { recursive: true })
