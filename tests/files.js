...
const { InvalidYearError, InvalidMonthError, InvalidDayError } = require("../src/errors");
...
describe("files.js()", () => {
	...
	describe("firstPositionInLogfile()", () => {
			expect(
				files.firstPositionInLogfile(
					"event-2019-01-31.log",
					"event-%y-%m-%d"
				)
	...
	describe("logfileDate()", () => {
		...
		it("should return false on files not matching the template", () => {
			expect(files.logfileDate("event-2019-01-01.txt", "event-%y-%m-%d.log")).to.equal(false);
		});

		it("should return false on dates that never existed", () => {
			expect(files.logfileDate("event-2019-13-01.txt", "event-%y-%m-%d.log")).to.equal(false);
			expect(files.logfileDate("event-2019-12-32.txt", "event-%y-%m-%d.log")).to.equal(false);
			expect(files.logfileDate("event-2019-02-29.txt", "event-%y-%m-%d.log")).to.equal(false);
		});
		...
		it("should return proper dates on %y %m %d order", () => {
			expect(files.logfileDate("event-2019-02-28.log", "event-%y-%m-%d.log"))
	...
	describe("logfileForDate()", () => {
		it("should generate log filenames with has parseable dates", () => {
			...
			expect(files.logfileDate(filename, template)).to.deep.equal({ year: 1999, month: 5, day: 1 });
		});
		it("should generate log filenames with has parseable dates", () => {
			const template = "data/%y/events-%d-%m.log";
			const filename = files.logfileForDate(1999, 5, 1, template);
			...
			return disposableFile.dir()
				.then(tmpDir => {
					return touch(path.join(tmpDir, "events-2012-01-31.log"))
						...
						.then(() => {
							return nextExistingLogfile(logfileForDate(2012, 1, 30, path.join(tmpDir, "events-%y-%m-%d.log")), path.join(tmpDir, "events-%y-%m-%d.log"));
						})
						.then(file => {
						...
						.then(() => {
							return nextExistingLogfile(logfileForDate(2012, 1, 31, path.join(tmpDir, "events-%y-%m-%d.log")), path.join(tmpDir, "events-%y-%m-%d.log"));
						})
						.then(file => {
							expect(file).to.equal(
								path.join(tmpDir, "events-2016-08-21.log"),
						...
						.then(() => {
							return nextExistingLogfile(logfileForDate(1977, 4, 15, path.join(tmpDir, "events-%y-%m-%d.log")), path.join(tmpDir, "events-%y-%m-%d.log"));
						})
						.then(file => {
							expect(file).to.equal(
								path.join(tmpDir, "events-1979-05-25.log"),
						...
						.then(() => {
							return nextExistingLogfile(logfileForDate(1979, 5, 24, path.join(tmpDir, "events-%y-%m-%d.log")), path.join(tmpDir, "events-%y-%m-%d.log"));
						})
						.then(file => {
							expect(file).to.equal(
								path.join(tmpDir, "events-1979-05-25.log"),
						...
						.then(() => {
							return nextExistingLogfile(logfileForDate(1979, 5, 25, path.join(tmpDir, "events-%y-%m-%d.log")), path.join(tmpDir, "events-%y-%m-%d.log"));
						})
						.then(file => {
							expect(file).to.equal(
								path.join(tmpDir, "events-1984-02-04.log"),
						...
						.then(() => {
							return nextExistingLogfile(logfileForDate(2012, 1, 30, path.join(tmpDir, "events-%d-%y-%m.log")), path.join(tmpDir, "events-%d-%y-%m.log"));
						})
						.then(file => {
							expect(file).to.equal(
								path.join(tmpDir, "events-31-2012-01.log"),
						...
						.then(() => {
							return nextExistingLogfile(logfileForDate(2012, 1, 31, path.join(tmpDir, "events-%d-%y-%m.log")), path.join(tmpDir, "events-%d-%y-%m.log"));
						})
						.then(file => {
							expect(file).to.equal(
								path.join(tmpDir, "events-21-2016-08.log"),
						...
						.then(() => {
							return nextExistingLogfile(logfileForDate(1977, 4, 15, path.join(tmpDir, "events-%d-%y-%m.log")), path.join(tmpDir, "events-%d-%y-%m.log"));
						})
						.then(file => {
							expect(file).to.equal(
								path.join(tmpDir, "events-25-1979-05.log"),
						...
						.then(() => {
							return nextExistingLogfile(logfileForDate(1979, 5, 24, path.join(tmpDir, "events-%d-%y-%m.log")), path.join(tmpDir, "events-%d-%y-%m.log"));
						})
						.then(file => {
							expect(file).to.equal(
								path.join(tmpDir, "events-25-1979-05.log"),
						...
						.then(() => {
							return nextExistingLogfile(logfileForDate(1979, 5, 26, path.join(tmpDir, "events-%d-%y-%m.log")), path.join(tmpDir, "events-%d-%y-%m.log"));
						})
						.then(file => {
							expect(file).to.equal(
								path.join(tmpDir, "events-04-1984-02.log"),
